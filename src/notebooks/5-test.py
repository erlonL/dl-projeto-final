...
# skip frames ≤ 25000
frame_paths = [p for p in all_frames
                if int(os.path.basename(p).split(".png")[0]) > 25000]
...
if __name__ == "__main__":
    # decide how many workers: by default #cpu cores
    num_workers = os.cpu_count() or 4

    # partial the arguments so our workers only need a single arg
    worker_fn = partial(_process_one,
                        score_thresh=score_thresh,
                        output_dir=output_dir)

    with ProcessPoolExecutor(max_workers=num_workers) as exe:
        # executor.map preserves order; you can also use as_completed() for unordered
        for frame_path, out_path, dets in tqdm(exe.map(worker_fn, frame_paths),
                                                total=len(frame_paths),
                                                desc="Processing frames"):
            # optional: display in notebook
            # note: display is in main process only, so it won't block workers
            img = Image.open(out_path)
            clear_output(wait=True)
            print("Processing:", frame_path)

    print("Done. Annotated frames are in", output_dir)